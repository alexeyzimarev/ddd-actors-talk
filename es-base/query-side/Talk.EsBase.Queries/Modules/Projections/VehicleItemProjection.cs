...
namespace Talk.EsBase.Queries.Modules.Projections
{
    public static class VehicleItemProjection
    {
        public static Func<Task> GetHandler(IAsyncDocumentSession session, object @event)
        {
            ...
            Task Update(string id, Action<VehicleItem> update)
                => session.UpdateItem(GetDbId(id), update);

            Task UpdateOneSensor(string id, string sensorId, Action<VehicleItem.VehicleSensor> update)
                => Update(id, vehicleItem =>
                    {
                        var sensor = vehicleItem.Sensors.FirstOrDefault(x => x.SensorId == sensorId);
                        if (sensor != null) update(sensor);
                    }
                );

            string GetDbId(string vehicleId) => VehicleItem.GetDatabaseId(vehicleId);
        }
    }
}