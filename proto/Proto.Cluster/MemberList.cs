...
namespace Proto.Cluster
{
    public static class MemberList
    {
        ...
        internal static string[] GetMembers(string kind)
        {
            ...
            while (!locked)
            {
                Logger.LogDebug("GetMembers: MemberList did not acquire reader lock within 1 seconds, retry");
        ...
        internal static string GetPartition(string name, string kind)
        {
            ...
            while (!locked)
            {
                Logger.LogDebug("GetPartition: MemberList did not acquire reader lock within 1 seconds, retry");
                locked = RwLock.TryEnterReadLock(1000);
            }
        ...
        internal static string GetActivator(string kind)
        {
            ...
            while (!locked)
            {
                Logger.LogDebug("GetActivator: MemberList did not acquire reader lock within 1 seconds, retry");
                locked = RwLock.TryEnterReadLock(1000);
            }
        ...
        internal static void UpdateClusterTopology(ClusterTopologyEvent msg)
        {
            ...
            while (!locked)
            {
                Logger.LogDebug("UpdateClusterTopology: yMemberList did not acquire writer lock within 1 seconds, retry");
