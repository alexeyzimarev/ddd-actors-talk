...
namespace Talk.Domain.Sensor
{
    public class SensorState : AggregateState<SensorState>
    {
        ...
        public override SensorState When(SensorState state, object @event)
            => @event switch
            {
                SensorInstalled e => With(state, x =>
                {
                    x.Id        = e.SensorId;
                    x.VehicleId = e.VehicleId;
                }),
                SensorTelemetryReceived e => With(state, x =>
                {
                    x.Speed       = e.Speed;
        ...
        public override string GetStreamName(string id) => $"Sensor-{id}";

        protected override bool EnsureValidState(SensorState newState)
            => !IsNullOrWhiteSpace(newState.Id) && !IsNullOrWhiteSpace(newState.VehicleId);
    }
}